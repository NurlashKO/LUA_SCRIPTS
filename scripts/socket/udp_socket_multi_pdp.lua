function test_udp_function(cid, server_address, peer_port)
  local timeout = 30000;
  ...
  print("network.open(", cid, "), app_handle=", app_handle, "\r\n");
...
local cmd = "AT+CGSOCKCONT=1,\"IP\", \"WRONGAPN\"\r\n"; 
sio.send(cmd);
sio.recv(5000);
cmd = "AT+CGSOCKCONT=2,\"IP\", \"3GNET\"\r\n"; 
sio.send(cmd);
sio.recv(5000);
cmd = "AT+CGSOCKCONT=3,\"IP\", \"CMNET\"\r\n"; 
sio.send(cmd);
sio.recv(5000);

local result;

local server_address = "116.247.119.165";
local peer_port = 990;

local cid = 2;--0=>use setting of AT+CSOCKSETPN. 1-16=>use self defined cid

test_udp_function(cid, server_address, peer_port);
